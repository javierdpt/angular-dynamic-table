...
export class UtilsService {
    ...
    getFilteredEntityOptions<T extends { [key: string]: any }>(
    ...
    getRandomItemsFromArray<T>(itemsCount: number, items: T[]): T[] {
        if (!items.length) { return []; }
        const resp: T[] = [];
        while (itemsCount-- > 0) {
            const itemsSubset = items.filter((item: T) => !resp.includes(item));
            resp.push(itemsSubset[this.getRandomInt(0, itemsSubset.length - 1)]);
        }
        return resp;
    }
